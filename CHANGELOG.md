...
- **"Eau chaude seulement" (DHW only) didn't actually activate - the MiGo app itself never showed it as active**, unlike Frost guard and Boost, even though writing it from Home Assistant succeeded and the entity itself displayed correctly. Root cause, found via a live capture of the app's own DHW-only action: it sets `temperature_control_mode: "cooling"` home-wide, alongside this room's `therm_setpoint_mode: "hg"` - the room-level write alone, all this used to send, never touched that flag. (Despite the name, this boiler line has no air conditioning; `"cooling"` is simply the flag this account's real DHW-only quick action runs under - an earlier investigation mistook a leftover `"cooling"` value for a broken, unreachable state and started forcing `"heating"` unconditionally on every home-level write, which happened to also work here as a side effect: it reliably clears a stale `"cooling"` value whenever Frost guard or Auto is selected afterward.) DHW-only, reached via either the preset or the climate wheel's Off position, now sets both fields together; Frost guard and Auto now go through the same `sethomedata` call as Away rather than the plain `setthermmode` endpoint, so they reliably clear a `"cooling"` value left over from DHW-only instead of risking the same 403
