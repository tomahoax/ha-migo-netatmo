...
- **`datetime.migo_{home}_away_until` silently rejected any value set on it** - same root cause as above: setting a return time activates Away via the same `set_home_therm_mode` call, so it hit the identical 403 and rolled back to empty, which looked like "no way to confirm the value" rather than a failed write. Fixed by the same change; confirmed no separate datetime-widget bug exists (no availability/editability coupling in `datetime.py` at all)
  - **Follow-up, caught live in the HA log right after the 403 fix shipped:** a second, distinct cause of the same symptom - the API also rejects a `therm_mode_endtime` that computes to a moment in the past outright (`400 {"error":{"code":21,"message":"endtime in past"}}`), which `_call_api_optimistically`'s rollback then silently cleared, again looking like "nothing happened" rather than a validation failure. A likely trigger: Home Assistant's own device-page compact date/time row has a related frontend bug (`RangeError: Invalid time value` in `ha-time-input.ts`/`hui-datetime-entity-row.ts`, visible in the same log capture) that can submit a partial edit before a full future moment has actually been picked - not something fixable from this integration; use the entity's full more-info dialog instead of the compact row as a workaround. `async_set_value` now validates the value is in the future and raises a clear `ServiceValidationError` before ever touching the API or the optimistic cache
