...
- **"Normal" climate preset**, the third option alongside "Hot water only" and "Frost guard" in the app's own "Actions rapides" - previously only reachable indirectly via HVAC mode Auto. Selecting it resets the room to its baseline (not overridden) and the home to `schedule`/`heating`, clearing any active Away, Frost guard or Hot water only in one action from the preset selector, without also needing the separate HVAC mode wheel
...
- **Renamed `datetime.migo_{home}_away_until`'s French name** from "Absent jusqu'à" to "Mode absent jusqu'à" so it sorts directly under the "Mode absent" switch on the device page (Home Assistant sorts entities within a card alphabetically by name)
...
- **Away mode couldn't be turned on - toggling it immediately reverted to off**, with a `403` from the API (`temperature_control_mode: cooling`). `MigoApi.set_home_therm_mode()` now sends `"temperature_control_mode": "heating"` for every mode except DHW-only, matching the MiGo app's own traffic. `datetime.migo_{home}_away_until` had the identical root cause and is fixed by the same change
- **"Eau chaude seulement" (DHW only) didn't actually activate - the MiGo app itself never showed it as active**, unlike Frost guard and Boost, even though writing it from Home Assistant succeeded and the entity itself displayed correctly. Root cause, found via a live capture of the app's own DHW-only action: it sets `temperature_control_mode: "cooling"` home-wide, alongside this room's `therm_setpoint_mode: "hg"` - the room-level write alone, all this used to send, never touched that flag. (Despite the name, this boiler line has no air conditioning; `"cooling"` is simply the flag this account's real DHW-only quick action runs under - an earlier investigation mistook a leftover `"cooling"` value for a broken, unreachable state and started forcing `"heating"` unconditionally on every home-level write.) A first fix attempt sent `temperature_control_mode: "cooling"` alongside a `therm_mode` value in the same request, matching every other mode change's shape - reported live and confirmed to get rejected with the identical 403 the original investigation was trying to avoid, regardless of whether `therm_mode` actually changed. `temperature_control_mode` is now set through its own request with no `therm_mode` field at all, never combined with a `therm_mode` change in the same call; Frost guard and Auto now go through `sethomedata` (`"heating"` alongside their `therm_mode`, the one combination confirmed to work) instead of the plain `setthermmode` endpoint, so they reliably clear a `"cooling"` value left over from DHW-only
...
- **Every thermostat-attached entity would have failed to set up on any real installation.** A `device_info` property linking a thermostat to its parent gateway was believed to need migrating off the `via_device` identifiers tuple (itself believed deprecated and at risk of raising instead of warning) to a `via_device_id` key holding the registry's own internal ID. Caught before release, against CI rather than a user: `via_device_id` is not a real `DeviceInfo` key in any released Home Assistant version, and `via_device` carries no deprecation notice at all in the same real, currently-supported versions - so every setup would have hit `TypeError: DeviceRegistry.async_get_or_create() got an unexpected keyword argument 'via_device_id'` for every thermostat-linked entity. Reverted to `via_device`, which Home Assistant itself resolves to the registry's internal ID. The parent gateway device is still registered up front before platforms are forwarded, so the link never races regardless of which form is used
...
- **The climate card lagged badly, sometimes indefinitely, after changing mode, preset or temperature.** It was the one entity never converted to the immediate-feedback pattern (`_call_api_optimistically`) every other write-capable entity already uses - every write just called the API then asked for a refresh, with no local state update at all. That refresh is debounced (10s cooldown), so clicking through modes, or just routine polling, regularly got coalesced into a no-op fetch, leaving the card stuck on stale data until the next real refresh landed. Now updates immediately, like everything else
- **Switching from Off to Auto could get stuck indefinitely.** Confirmed against a live debug-log capture: `therm_setpoint_mode` stayed at `"hg"` through repeated Auto clicks. Selecting Auto only ever changed the home-level mode, never this room's own leftover `"hg"` from a previous Off/DHW-only selection - and the climate entity checks the room-level value first, so it kept showing Off no matter how many times Auto was selected (Heat happened to work, since it writes room-level data too). Auto and the Frost guard preset now also clear a leftover room-level override first when one is active
- **The DHW-only preset could read back as Frost guard after a previous Frost guard selection**, even though the write itself was correct. Selecting DHW-only only ever changed this room's mode, never the home-level one left over from an earlier Frost guard - and the climate entity checks the home-level value first, so a stale "hg" there kept showing Frost guard regardless of what was picked afterward. DHW-only now clears it first when needed, without touching an active Away (which is meant to combine with DHW-only, unlike Frost guard)
- **`sensor.migo_{home}_boiler_mode`, the Away mode switch and binary sensor, and the DHW schedule and DHW always-on entities all showed one fixed icon regardless of state**, standing out against the rest of the integration's icons, which already change with state. Each had a static icon set in code, which always overrides an icon translation - moved to `icons.json`'s per-state mapping instead: Normal/DHW only/Frost guard each get their own icon, and the on/off entities now visually distinguish their two states
- **Restored the Platinum `strict-typing` mypy baseline (0 errors), silently broken by this branch's own refactor.** Caught by CI, not by a local check: this sandbox's own Python environment had an unconstrained, unreleased Home Assistant snapshot installed, which happened to mask every one of these against real, currently-released versions. Mostly missing `@override` decorators on methods the refactor's shared mixins introduced, and a handful of `dict[str, Any]`-typed helper parameters that don't accept the TypedDicts this integration actually passes them (`is_home_away`, `get_event_schedule`, `get_rooms_for_home`, now `Mapping[str, Any]`/the precise TypedDict instead) - no behavior changes, type-checking only
