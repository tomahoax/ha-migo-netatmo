...
class TestResolveViaDeviceId:
    ...
    Regression guard: `via_device` (the identifiers-tuple form) has been
    removed from `DeviceInfo` entirely as of the versions this integration
    now targets - see `entity_device_info.py`'s module docstring for the
    full back-and-forth history. All four `device_info` properties that
    link to a parent gateway now resolve `via_device_id` through this
    function instead.

    Uses `async_get_device_by_identifier` (config-entry-scoped), not
    `async_get_device` - the latter is ambiguous across config entries and
